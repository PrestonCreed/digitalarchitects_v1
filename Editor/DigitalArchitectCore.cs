...
{
    // UI Manager class that was missing
    public class DigitalArchitectManager : MonoBehaviour
    {
        public event System.Action<string> OnUserMessageSent;
        
        public void ShowAgentMessage(string message)
        {
            Debug.Log($"Agent Message: {message}");
            // Implement UI display logic
        }

        public void UpdateEnvironmentDisplay(string key, object value)
        {
            Debug.Log($"Environment Update - {key}: {value}");
